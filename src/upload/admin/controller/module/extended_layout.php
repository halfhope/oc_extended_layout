...
class ControllerModuleExtendedLayout extends Controller {
	...
	public function form() {
		...
		if (version_compare('2.0.3.1', VERSION) < 0) {
			...
			$this->customer_groups_model = $this->model_customer_customer_group;
		} else {
			$this->load->model('sale/customer_group');
			$this->customer_groups_model = $this->model_sale_customer_group;
		}
		...
		$data['customer_groups'] = $this->customer_groups_model->getCustomerGroups();
		...
		if (version_compare('2.2', VERSION) >= 0) {
			$this->response->setOutput($this->load->view($this->_route . '.tpl', $data));
		} else {
			...
			if ($settings['customer_group_id']) {
				...
				foreach ($settings['customer_group_id'] as $customer_group_id) {
					if ($customer_group_id == null) {
						...
						$customer_group_info = $this->customer_groups_model->getCustomerGroup($customer_group_id);
